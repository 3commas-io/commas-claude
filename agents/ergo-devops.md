...
- `process_list node=X min_mailbox=1 min_uptime=60 min_messages_in=1 sort_by=mailbox_latency limit=10` -- liveness candidates (exclude zombees from results, compute `RunningTime_ns / (Uptime_s * MailboxLatency_ns)`: >0.01 = overloaded but alive, <0.001 = callback blocked)
